...
Before routing a request, `central-automation-orchestrator` runs the `framework-discovery` Skill (`node .github/skills/framework-discovery/detect.js`) to identify the repository's tech stack from static evidence only (`package.json`, config files, directory structure — never a browser/MCP session). Result is cached at `index/framework-profile.json`, keyed by a signature hash of the evidence it read; only regenerated when that evidence changes. See [.github/skills/framework-discovery/README.md](.github/skills/framework-discovery/README.md).

`framework-discovery` only ever checks static evidence it can verify without guessing — dependency manifests, config files, and this scaffold's own out-of-the-box `page-objects/` folder name. That last check is real evidence for a repo already shaped like this one, but not for a repo laid out differently (a different top-level folder name, or a Maven/Gradle Java project with no fixed top-level folder for this at all) — rather than growing a per-framework list of folder names to guess inside `detect.js`, that judgment is delegated to the `repo-convention-discovery` Skill: an agent that reads the repo's real source files once and writes what it finds into `framework-profile.json`'s `conventions` field (`pageObjectsPath`/`apiClientsPath`/`testsPath`). `detect.js` prefers `conventions.*` when present and only falls back to its own `page-objects/`-folder check when absent — regenerating the profile (`--force`) never erases a previously recorded `conventions` block. See [.github/skills/repo-convention-discovery/README.md](.github/skills/repo-convention-discovery/README.md); it's invoked as part of the same calibration pass documented in `ADOPTION.md` §3.
...
- `agents/api-automation-specialist.agent.md` — same pattern for API tests.
- `agents/automation-healer-specialist.agent.md` — diagnoses a failing test and applies the smallest safe fix; delegates CI evidence gathering to `ci-analyzer-specialist`.
- `agents/ci-analyzer-specialist.agent.md` — read-only Azure DevOps (ADO) pipeline build analysis.
...
### Intelligent reuse layer: `.github/capabilities/` + mirrored class index
...
- **Index**: `index/page-objects/<className>.json`, one file per Page Object mirroring `page-objects/` (e.g. `index/page-objects/loginPage.json` for `page-objects/loginPage.js`) - `className`, `file`, `methods: [{name, params}]`.
- **Authorship**: hand-authored (or AI-assisted), not mechanically parsed from source - this is what makes the same mechanism work regardless of the target project's language (JS, TS, Java, ...). `page-object-generator` writes/updates the index file itself whenever it creates or modifies a Page Object, since it already knows the methods it just wrote; a pre-existing codebase the pipeline didn't generate is indexed by a one-off manual/AI-assisted pass instead. Keeping an index entry current when its Page Object changes is the same discipline as keeping a test current - no separate generation or verification step exists for it, and none is planned; see [page-object.instructions.md](.github/instructions/page-object.instructions.md).
- **Search**: `.github/capabilities/search-simplified.js` (`ClassIndexSearch`) — `searchClasses(query)`, `searchMethods(query)`, `analyzeReuseCoverage(requiredActions)` (returns REUSE ≥100% / PARTIAL 50–99% / EXPLORE <50% coverage with `skipExploration`/`explorationScope`). CLI: `node .github/capabilities/search-simplified.js "<query>"`.
...
- **Deliberately excluded**: no `capabilities/login.json`/`create.json`-style business-capability abstraction layer — that was built (v1/v2), found over-engineered, and removed in favor of direct class/method search (v3.0). Do not reintroduce a capability-name indirection layer. A JS-only source parser and generated `artifacts/indexes/classes/`/`_manifest.json` replaced it (v3.0) and was itself removed (v4.0) in favor of the mirrored, manually-authored index above, since the parser only ever worked for JavaScript.
