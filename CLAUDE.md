...
- `agents/unified-test-orchestrator.agent.md` — entry-point router; classifies the request (Jira ID, Epic, Swagger, URL, failing test, etc.) and delegates to one specialist agent below. Never generates artifacts itself.
...
- `prompts/*.prompt.md` — parameterized entry points bound to one agent: `start-test-automation` -> orchestrator, `generate-playwright-test` -> test-generator-ui, `generate-api-test` -> test-generator-api, `heal-playwright-test` -> unified-test-healer.
