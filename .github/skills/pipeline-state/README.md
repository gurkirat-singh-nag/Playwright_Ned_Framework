...
- `qa-dispatcher.agent.md`: unchanged - it does not own the per-story pipeline and does not need pipeline-state awareness, consistent with its existing "no pipeline artifact" boundary (see `CLAUDE.md`/agent docs from Prompt 1-4).
