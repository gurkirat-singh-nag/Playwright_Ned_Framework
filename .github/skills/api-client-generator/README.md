...
- `artifacts/indexes/api/*.json` (existing client index, via `api-capability-discovery`'s search)
...
2. For each operation, check `artifacts/indexes/api/` (via `api-capability-discovery`'s `ApiIndexSearch`) for an existing client method that already covers it; if found and source-verified, reuse it - never generate a duplicate.
