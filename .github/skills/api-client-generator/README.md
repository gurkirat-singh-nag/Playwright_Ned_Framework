...
- `<clients|api|services>/*.index.json` (existing client index, via `api-capability-discovery`'s search)
...
- `<className>.index.json` colocated with each client class this Skill creates or modifies
...
2. For each operation, check the colocated `*.index.json` files (via `api-capability-discovery`'s `ApiIndexSearch`) for an existing client method that already covers it; if found, reuse it - never generate a duplicate.
...
7. Write the client file(s), and write or update the matching `<className>.index.json` colocated with each one - see [api-client.instructions.md](../../instructions/api-client.instructions.md). This Skill already knows exactly which methods it just wrote, so authoring the index entry here costs nothing extra and needs no source parsing.
