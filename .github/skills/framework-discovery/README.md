...
- By `qa-dispatcher`, early in the pipeline - after understanding the raw request, before delegating to a UI/API specialist agent (`test-generator-ui`, `test-generator-api`) - so the dispatcher (and, transitively, the agent it delegates to) already knows what kind of framework it's generating into.
...
- `qa-dispatcher` calls `node .github/skills/framework-discovery/detect.js` (no flags - it self-detects staleness) before determining the UI/API generation path, and reads the resulting `artifacts/indexes/framework-profile.json` into its context package alongside the routing decision it hands off.
