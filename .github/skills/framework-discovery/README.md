...
- Automatically, whenever `index/framework-profile.json` is missing or stale (see Staleness below) - the Skill self-detects this, so callers do not need to check first.
...
- Directory listings: `page-objects/`, `tests/`, `utils/`, `api/`, `clients/`, `fixtures/`, `.github/{agents,skills,hooks,workflows}/`

## What It Detects

See `index/framework-profile.json` schema below. Ten categories: language, UI automation technology, API automation technology, test runner, automation architecture (UI and API separately), important directory locations, build/package management, reporting, CI/CD, and MCP configuration.
...
A single artifact: `index/framework-profile.json`.
...
```json
{
  ...
  "conventions": {
    "pageObjectsPath": "...", "apiClientsPath": "...", "testsPath": "..."
  }
}
```
...
`conventions` is different from every other field above: this Skill never writes it (only ever reads and preserves it across regeneration). It's populated by the `repo-convention-discovery` Skill - an agent that reads real source files and records where they actually live, for repo shapes this Skill has no static evidence for (a Maven/Gradle Java project has no fixed top-level folder to check the way `page-objects/` is checked here). `detectUiArchitecture()`, `detectApiArchitecture()`, and `detectDirectories()` all prefer `conventions.*` when present, and fall back to this Skill's own evidence-checked defaults (a top-level `page-objects/` folder, presence-only) only when it's absent. `conventions` is `null` until `repo-convention-discovery` has run at least once for this project - that's an expected, valid state, not an error.
...
2. If `index/framework-profile.json` is missing, or its stored `signature` doesn't match, the profile is stale → regenerate.
...
- `central-automation-orchestrator` calls `node .github/skills/framework-discovery/detect.js` (no flags - it self-detects staleness) before determining the UI/API generation path, and reads the resulting `index/framework-profile.json` into its context package alongside the routing decision it hands off.
...
- If `index/` cannot be written to (permissions, read-only filesystem): WARN and continue with an in-memory profile; downstream agents fall back to per-Skill detection as if no profile existed.
