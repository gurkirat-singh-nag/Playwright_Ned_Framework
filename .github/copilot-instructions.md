...
| [api-capability-discovery](skills/api-capability-discovery/README.md) | `test-design.json` (API/both-scoped scenarios) → `api-reuse-decision.json`: searches colocated `*.index.json` files next to each client class for existing API clients/methods, decides FULL_REUSE/PARTIAL_REUSE/NO_REUSE per scenario. No Swagger fetch, no live API call, no MCP - only clears the way for conditional Swagger/API exploration next. API equivalent of the UI reuse-enforcement hook. |
