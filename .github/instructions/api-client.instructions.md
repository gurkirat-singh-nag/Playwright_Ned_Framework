...
- Every client class has a matching index file at `index/clients/<className>.json` (or `index/api/`/`index/services/`, mirroring whichever of `clients/`/`api/`/`services/` the class lives in), listing its class name, file path, and public methods (name + params). Update it in the same change whenever a method is added, removed, or renamed - this is the reuse index `api-capability-discovery` searches before Swagger/live API exploration; nothing else keeps it in sync automatically.
